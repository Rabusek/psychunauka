...
export default function Sidebar({ activeMode, onModeChange, isOpen, onToggle }: SidebarProps) {
  ...
  const handleModeClick = (mode: 'learn' | 'exam' | 'stats' | 'ranking' | 'materials') => {
    onModeChange(mode);
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:relative w-64 bg-white shadow-lg flex flex-col border-r border-gray-200 h-screen z-40 transition-transform duration-300 ${
      ...
      >
        ...
        <nav className="flex-1 p-3 overflow-y-auto">
          <div className="space-y-1">
            <button
              onClick={() => handleModeClick('materials')}
            ...
            <button
              onClick={() => handleModeClick('ranking')}
              className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg transition text-sm ${
                activeMode === 'ranking'
                  ? 'bg-indigo-50 text-indigo-700 font-medium'
                  : 'text-gray-700 hover:bg-gray-100'
